...
function zb_zbapp_admin_init(){
    ...
    /* Register js. */
	wp_register_script( 'zb_zbapp_admin_init', plugins_url('assets/js/main.js?v='.time(), __FILE__) );
...
function update_zb_zbapp_code(){
	if(isset($_REQUEST['token']) && wp_verify_nonce($_REQUEST['token'], 'update_zb_zbapp_code')){
		...
		if(!isset($domain) || empty($domain)){
			...
			if(wp_verify_nonce($_REQUEST['token'], 'update_zb_zbapp_code')){
