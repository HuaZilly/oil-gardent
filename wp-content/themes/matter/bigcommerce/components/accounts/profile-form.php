...
// Remove fake address that enabled
if ($first_name === '##TO-BE-FILLED##') :
	$customer = new Customer( $user_id );
	$addresses = $customer->get_addresses();
	foreach ($addresses as $address) :
		$customer->delete_address( $address['id'] );
	endforeach;
endif;
	...
	<div class="inner">
		...
		<form class="bc-form bc-form-2col bc-account-form--profile bc-account-profile <?php if ( ! empty( $errors ) ) { echo 'bc-form--has-errors'; } ?>" action=""
			...
			<label class="bc-form__control bc-form-2col__control bc-form-2col__control--left <?php if ( in_array( 'first_name', $errors ) ) { echo $error_class; } ?>" for="bc-account-profile-firstname">
				...
				<input type="text" name="bc-profile[first_name]" id="bc-account-profile-firstname" value="<?php echo esc_attr( $first_name === '##TO-BE-FILLED##' ? '' : $first_name ); ?>" data-form-field="bc-form-field-first_name">
			</label>
			...
			<label class="bc-form__control bc-form-2col__control bc-form-2col__control--right <?php if ( in_array( 'last_name', $errors ) ) { echo $error_class; } ?>" for="bc-account-profile-lastname">
				...
				<input type="text" name="bc-profile[last_name]" id="bc-account-profile-lastname" value="<?php echo esc_attr( $last_name === '##TO-BE-FILLED##' ? '' : $last_name ); ?>" data-form-field="bc-form-field-last_name">
			</label>
			...
			<label class="bc-form__control bc-form-2col__control bc-form-2col__control--left <?php if ( in_array( 'company', $errors ) ) { echo $error_class; } ?>" for="bc-account-profile-company">
				...
				<input type="text" name="bc-profile[company]" id="bc-account-profile-company" value="<?php echo esc_attr( $company === '##TO-BE-FILLED##' ? '' : $company ); ?>" data-form-field="bc-form-field-company">
			</label>
			...
			<label class="bc-form__control bc-form-2col__control bc-form-2col__control--right <?php if ( in_array( 'phone', $errors ) ) { echo $error_class; } ?>" for="bc-account-profile-phone">
				...
				<input type="tel" name="bc-profile[phone]" id="bc-account-profile-phone" value="<?php echo esc_attr( $phone === '00000' ? '' : $phone ); ?>" data-form-field="bc-form-field-phone">
			</label>
			...
			<div class="bc-form__actions bc-account-profile__actions">
				<button class="btn primary bc-btn--account" aria-label="<?php __( 'Update Details', 'bigcommerce' ); ?>" type="submit"><?php echo esc_html( __( 'Update Details', 'bigcommerce' ) ); ?></button>
			</div>
		</form>
	</div>
</section>
